view: offline_pricing_darkstore {
  derived_table: {
    sql: select o.id ,
  ...
  on sp_p.id = (
    ...
    where sv.id = i.variant_id
      ...
      and s.spree_zone_id in (97,96, 155, 164, 165, 166, 167)
      group by
      ...
      and i.quantity > 0
      and s.spree_zone_id in (97,96, 155, 164, 165, 166, 167)--,98,100,101,102,103)
      and COALESCE(o.parent_id,-1) = -1
...
##}
  ...
  dimension: supermarket_id {
    case: {
      ...
      when: {
        sql: ${TABLE}.supermarket_id  = 165 ;;
        label: "57502"
      }

      when: {
        sql: ${TABLE}.supermarket_id  = 166 ;;
        label: "57501"
      }

      when: {
        sql: ${TABLE}.supermarket_id  = 167 ;;
        label: "57003"
      }
